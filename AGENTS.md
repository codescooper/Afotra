...
- **Focus guard** (only while a session is **Running**): each 800 ms overlay tick samples the
  ...
  **allow-list** (`OutilsTache`) nor session-snoozed, the orb turns **Ask** and the `OrbAskPopup`
  asks *« … en rapport avec la tâche ? »*. It is **relentless and escalating**: intensity grows
  with the off-task streak (`GuardOffTaskSince`, ~25 s to max) **and** each *Non* (`GuardNonCount`,
  +0.3) → the orb gets bigger/redder/faster with a stronger glow (`Get-OrbVisual Ask` scales size,
  `PulsePeriodMs`, `GlowRadius`, wobble with `Intensity`).
  - **Oui** → `Add-TaskTool` (process joins the task's allow-list, persisted) → `Clear-Guard`, orb calms.
  - **Non** → does **not** calm: `Add-TaskDigression`, bump `GuardNonCount`, hide the popup for a
    5 s re-arm (`GuardReaskAt`) then ask again — **the guard stays armed** so the orb keeps
    escalating until you actually return to an allowed process. After 2 *Non* it also starts the
    audible alarm (regardless of `guardSound`).
  - **Ignorer** → in-memory snooze for the rest of the session (no lockout; not persisted).
  Returning to an on-task/allowed process is the **only** thing that resets it (`Clear-Guard`
  zeroes the streak + `GuardNonCount`). The guard **never touches the offending window** (pressure only).
...
## Visual theme (dark + honey, bee mascot)

The whole app is **dark** (`#222222` bg, sidebar `#1A1A1A`, cards `#2B2B2B`/border `#383838`,
text `#F5F5F5`/`#9A9A9A`) with **honey-yellow `#FFC107`** as the brand accent (dark text `#1A1A1A`
on yellow) and a **bee mascot**. Sidebar has a vector-bee logo; the active nav item is recoloured
yellow in `Show-View`→`Set-NavActive`. The Dashboard front is metric **tiles** (icon + number +
accent bar) + a top bar with global search (`GlobalSearchBox` → routes to the Tâches filter).
`DataGrid`/`TextBox`/`ListBox` get dark **implicit styles** in `Window.Resources`; the Tâches grid's
`Add_LoadingRow` uses dark row colours (dark-red/amber/dim). WinForms dialogs are dark-themed via
`Set-DarkTheme -Control $form` (called before every `ShowDialog`).

The assistant overlay is a **bee** (`OrbViewbox`: honey body + black stripes + flapping wings),
driven by the same `Orb.Core` mood mapping — now a **honey/amber → orange → red** palette
(Idle `#FFB300`, Focus `#FFA000`, Overrun `#F4511E`, Ask `#EF4444` escalating, Resume `#FFD740`).
